...
//use tantivy::query::{AllQuery, FuzzyTermQuery, RangeQuery, TermQuery};
//use tantivy::schema::*;
//use tantivy::DocAddress;
//
//use crate::message::{get_id, Message};
...
//        //Will break when new takes an option
//        let source = Maildir::from(self.src.as_ref().unwrap().clone());
//
//        Indexer {
//            index,
//            source: Some(source),
...
//    }
//
//    pub fn new_idx(index: PathBuf) -> Indexer {
//        Indexer {
...
//    }
//
//    pub fn tag_doc(&self, doc: Document, tags: Vec<String>) -> Result<(), tantivy::Error> {
//        let mut index_writer = self.get_index_writer(1);
//        let id = TantivyMessage::from(doc, &self.index.email).id;
//        let term = Term::from_field_text(self.index.email.id, id.as_ref());
//        index_writer.delete_term(term.clone());
//        println!("Commiting delete on {}", id);
//        index_writer.commit()?;
//        self.index.index.load_searchers()
//    }
//
//    fn mail_iterator(
//        source: &Maildir,
...
//            error!("Can't index mails with an index writer without a source");
//            return;
//        }
//        let (tx, rx) = mpsc::channel();
//        let mails: Vec<Result<MailEntry, _>> =
//            Indexer::mail_iterator(self.source.as_ref().unwrap(), full).collect();
//        let count = mails.len();
...
//                            let hash = get_id(
//                                unparsed_msg.data().expect("Unable to read the actual data"),
//                            );
...
//        }
//        index_writer.commit().expect("Can't commit for some reason");
...
//    }
//
//    pub fn latest(&self, num: usize, after: Option<u32>) -> Vec<TantivyMessage> {
...
//            ret.push(TantivyMessage::from(retrieved_doc, &self.index.email));
//        }
//        ret
//    }
//
...
//            Term::from_field_text(self.index.email.id, id.as_ref()),
...
//        match doc {
//            Ok(doc) => Some(TantivyMessage::from(doc, &self.index.email)),
//            Err(_) => None,
//        }
//    }
//    pub fn fuzzy(&self, term: &str, num: usize) -> Vec<TantivyMessage> {
//        let searcher = self.index.index.searcher();
